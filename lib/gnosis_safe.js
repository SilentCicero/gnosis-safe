...
function openIpfs() {
  return ipfsAPI('ipfs.infura.io', '5001', {
    protocol: 'https'
  })
}
...
function formatNumber(number) {
  var string = new BigNumber(number).toString(16)
...
function addDescription(safeAddress, transaction, transactionHash, subject) {
  var data = JSON.stringify({
    ...
    "nonce": formatNumber(transaction.nonce),
...
export default class {
  ...
  // npm run gscli -- confirm 0xa5056c8efadb5d6a1a6eb0176615692b6e648313 0x13de59675d22fb7befd213324f3f829135c6e8f8 0x9bebe3b9e7a461e35775ec935336891edf856da2 1 0x 0 2 something
  confirmTransaction(safeAddress, transaction, subject) {
    ...
    return safe.getTransactionHash.call(transaction.to, transaction.value, transaction.data, transaction.operation, transaction.nonce)
      .then((transactionHash) => {
        addDescription(safeAddress, transaction, transactionHash, subject)
          .then((hash) => {
